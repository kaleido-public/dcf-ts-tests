...
import { ObjectManagerImpl, ObjectManager, Ajax, CollectionManager } from 'django-client-framework-typescript';
...
describe('related collection manager tests', () => {
    ...
    //     await Axios.post('http://localhost:8000/brand/1/products', [5]);
    ...
    //     // await http.post(uri, body: data, headers: {'content-type': 'application/json'});
    //     await Axios.post('http://localhost:8000/brand/1/products', [5, 6]);
    //     try {
    //       var related_product = await bom.products.get({});
    //       assert.fail("shouldn't have got here");
    //     } catch (error) {
