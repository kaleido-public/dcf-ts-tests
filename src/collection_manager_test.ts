...
describe('collection manager tests', () => {
    beforeEach(async () => {
        // Axios.defaults = {...Axios.defaults, baseURL: "http://localhost:8000"}
        await Axios.get('http://localhost:8000/subapp/clear');
    });
    ...
    it('test get object none should fail', async () => {
        ...
        try {
          ...
          expect(error.message).to.equal('.get() must receive exactly 1 object, but got 0.');
        }
    });
    ...
    it('test get object more than one should fail', async () => {
      ...
      try {
        ...
        expect(error.message).to.equal( '.get() must receive exactly 1 object, but got 2');
      }
    });
    ...
    it ('test page page 1', async () => {
      ...
      assert.equal(pr.total, 51);
      assert.equal(pr.page, 1);
      assert.equal(pr.objects.length, 50);
      assert.equal(pr.objects[0].id, 1);
      expect(pr.objects[0].barcode).to.equal("product 1");
      assert.equal(pr.objects[49].id, 50);
      expect(pr.objects[49].barcode).to.equal("product 50");
    });
    ...
    it ('test page page 2', async () => {
      ...
      var pr = await cm.page({"page": {"page": 2}});
      assert.equal(pr.limit, 50);
      assert.equal(pr.total, 51);
      assert.equal(pr.page, 2);
      assert.equal(pr.objects.length, 1);
      assert.equal(pr.objects[0].id, 51);
      expect(pr.objects[0].barcode).to.equal("product 51");
    });
    ...
    it ('test page query', async () => {
      ...
      var pr = await cm.page({"query": {"barcode__in": ["product 1", "product 3", "product 5"]}});
      assert.equal(pr.limit, 50);
      assert.equal(pr.page, 1);
    ...
    it ('test page query with page', async () => {
      ...
      var pr = await cm.page({"query": {"id__in": [3, 5], "barcode__in": ["product 1", "product 3", "product 5"] }, "page": {"limit": 1, "page": 2}} );
    ...
    it ('test page query order by', async () => {
      ...
      var pr = await cm.page({"query": {"barcode__in": ["shoe 1", "shoe 3", "shoe 5"] }, "page": {"order_by": "-barcode"}} );
      assert.equal(pr.limit, 50);
      assert.equal(pr.page, 1);
      assert.equal(pr.total, 3);
      assert.equal(pr.objects.length, 3);
    ...
    it ('test page query order by ver 2', async () => {
      ...
      var pr = await cm.page({"query": {"barcode__in": ["product 1", "product 4", "product 5"] }, "page": {"order_by": "-barcode,-id"}} );
      assert.equal(pr.limit, 50);
      assert.equal(pr.page, 1);
    ...
    it ('test page typo', async () => {
      ...
      try {
        var pr = await cm.page({"query": {"barcode___in": ["product 1", "product 4", "product 5"] }, "page": {"order_by": "-barcode,-id"}} );
      } catch (error) {
    ...
    it('test get or create', async () => {
      var cm = new CollectionManager(Product);
      var om = await cm.get_or_create({"query": {"barcode": "product 1"}, "defaults": {"brand_id": null} });
    ...
    it('test get or create v3', async () => {
      ...
      var om = await cm.get_or_create({"query": {"barcode": "product 2"}, "defaults": {"brand_id": null}});
    ...
    it('test update or create', async () => {
      var cm = new CollectionManager(Product);
      var om = await cm.update_or_create({"query": {"barcode": "product 2"}, "defaults": {"barcode": "product 3"}});
